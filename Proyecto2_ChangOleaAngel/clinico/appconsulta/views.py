...
class NuevaRecetaPacientes(CreateView):
    ...
    template_name = "atencion/receta/registrar_receta.html"
...
class DetalleRecetaPacientes(CreateView):
    ...
    template_name = "atencion/receta/registrar_detalleReceta.html"
    success_url = reverse_lazy('base:paciente')

class DetalleRecetas(View):
    template_name = 'atencion/receta/detalleReceta.html'
    def get(self, request, *args, **kwargs):
        data = {}
        pk = request.GET.get("id_rec", "")
        recetaD = DetalleReceta.objects.filter(receta=int(pk))
        data['detRecetas']=recetaD
        return render(request, self.template_name, data)

#TOMA SIGNOS VITALES
class SignosPacientes(View):
    template_name = 'atencion/receta/tomaSigVitales.html'
    def get(self, request, *args, **kwargs):
        data = {}
        pk = request.GET.get("id_rec", "")
        signos = TomarSignoVital.objects.filter(receta=int(pk))
        data['signos']=signos
        return render(request, self.template_name, data)
...
class TomaSignosPacientes(CreateView):
    ...
    template_name = "atencion/receta/registrar_tomaSigVitales.html"
    success_url = reverse_lazy('base:paciente')
